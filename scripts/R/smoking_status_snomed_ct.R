...
setwd("Z:/Group_work/Harley/Codelists/code_translation")
...
df <- read.dta13("Z:/Database guidelines and info/SNOMED CT/SNOMED_CT_Description_ID_Term_v37.0.0_20230927.dta")
...
searchMedBro <- function(df, searchlist, searchin = c("term")) {
  ...
  for (i in names(searchlist)) {
    ...
    tempdat <- data.frame(V1 = grepl(paste0(searchlist[[i]], collapse = "|"), df[[searchin[1]]], ignore.case = TRUE))
    ...
    if (length(searchin) > 1) {
      ...
      for (j in searchin[2:length(searchin)]) {
        ...
        #  tempdat[[j]] <- grepl(paste0(searchlist[[i]], collapse = "|"), df[[searchin[j]]], ignore.case = TRUE)
        tempdat[[j]] <- grepl(paste0(searchlist[[i]], collapse = "|"), df[[j]], ignore.case = TRUE)
